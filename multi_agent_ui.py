...
class GlobalStatsBar(Static):
    ...
    def refresh_stats(self, states: list[AgentState], start: float) -> None:
        ...
        sonnet_cost = sum(
            (s.input_tokens * 3.0 + s.output_tokens * 15.0) / 1_000_000 for s in states
        )
...
class TeamAgentApp(App[None]):
    ...
    def _tick(self) -> None:
        self.query_one("#global-stats", GlobalStatsBar).refresh_stats(self._states, self._start)
    ...
    async def _auto_compress(self, idx: int) -> None:
        ...
        try:
            ...
            summary = next((b.text for b in resp.content if b.type == "text"), "(no summary)")
        except Exception:
...
def main() -> None:
    ...
    for a in agents:
        ...
        p = PRICING[a.model]
        print(
            f"  {a.name:12s} {model_short:20s}  budget {a.budget // 1000}K  ${p['input']:.2f}/M input"
        )
    print()
